...
import os
import sys
#apt install  freetds-bin python3-tds tdsodbc
...
class astm_file(file_mgmt):
  ...
  def mk_tuple(self):
    ...
    for x in each_line:
      if len(x)>0:
        if   x[0]=='H':
          self.on_header(x)             #set delimiters, equipment_id
        elif x[0]=='P':
  ...
  def on_header(self,header_line):
    ...
    try:
      self.analyser_id=header_tuple[4]  #5.1 ^ 5.2
      astr='Analyser Id:({aid})'.format(aid=self.analyser_id)
      logging.debug(astr)
    except Exception as my_ex:
      pstr='Look at H record <<<{}>>> Is it inappropriate? no header_tuple[4] found'.format(header_tuple)
      logging.debug(pstr)
      logging.debug(my_ex)
  ...
  def on_termination(self,termination_line):
    ...
    #no need to send it to final data
...
if __name__=='__main__':
  ...
  while True:
    ...
    if(m.get_first_inbox_file()):
      m.analyse_file()
      m.mk_tuple()
      # send to database function specific for each equipment/lis
